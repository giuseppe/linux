...
static int erofs_ioctl_get_source_fd(struct file *filp)
{
	...
	/* Require init-userns CAP_SYS_ADMIN so that a userns admin cannot
	 * obtain an fd to a backing file it would not otherwise be able to access. */
	...
	f = dentry_open(&sbi->dif0.file->f_path, O_RDONLY, current_cred());
	if (IS_ERR(f)) {
		put_unused_fd(fd);
		return PTR_ERR(f);
	}
